...
func _on_skip_pressed() -> void:
	...
	# Fade audio down to avoid pop, then stop video, reset volume, and advance.
	var tween := create_tween()
	tween.tween_property(video_player, "volume_db", -80.0, 0.2)
	tween.tween_callback(func() -> void:
		video_player.stop()
		video_player.volume_db = 0.0  # Reset for next video
